...
- [x] 1.1 Add `src/pages/not-found/index.tsx`. Compose `DefaultLayout` and `Header`, then a centered
      `BaseCard` inside a `min-h-svh` grid, following `src/pages/contact/index.tsx`. No local state.
- [x] 1.2 Copy the landscape scene markup from the contact page: the dawn and dusk cloud layers, the
...
- [x] 1.3 Add a comment at the top of the file recording that the scene is duplicated on purpose,
...
- [x] 1.4 Add the copy: `AutoH` + `BaseT1` reading "Nothing built here", then a `BaseT4` display
...
- [x] 1.5 Add one `AppLink` to `/portfolio`, and nothing else. No search, no suggested links, no
      second call to action.

## 2. The route

- [x] 2.1 In `src/App.tsx`, import the page and add `{ path: '*', element: <NotFoundPage /> }` as the
      last child route.

## 3. Checks

- [x] 3.1 Format and lint only the files this change touched: `pnpm exec oxfmt <paths>` and
