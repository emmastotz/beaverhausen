...
type Props = {
  ...
  renderScene?: (lineIndex: number) => ReactNode
}
...
export function ScrollRevealTextV2({
  lines,
  className = '',
  variant = 'body',
  persistLast = false,
  ...
  return (
    <div className="relative w-full">
      {lines.map((line, i) => {
        ...
        const scene = renderScene?.(i)

        return (
          <div key={i} className="relative size-full">
            {scene && (
              <div className="absolute inset-0 overflow-clip">{scene}</div>
            )}
            <div className="relative flex h-screen w-full items-center justify-center">
              <div className="z-raised mx-auto flex size-full max-w-2xl items-start justify-center text-center">
                <span
